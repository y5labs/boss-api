...
inject('pod', async ({ app, hub, db, log, startup, boss }) => {
  ...
  const load_jobs = async ({ queue, statuses, currentpage, itemsperpage = 25, search_term = null }) => {
    ...
    const res = search_term && search_term.length > 2
      ? await db.query(`
        select
          id,
          data,
  ...
  hub.on('socket disconnected', socket => { sockets.delete(socket) })
  ...
  const handle = setInterval(async () => {
    ...
    const states_map = Object.entries(summary.reduce((map, i) => {
      const name = i.name ?? `${boss_prefix}total_all`
      const state = i.state ?? 'total'
      if (!map[name])
        map[name] = Object.fromEntries(Object.keys(boss_states).map(q => [q, 0]))
    ...
    last.special.push(['total', last.queues.reduce((obj, [_, q]) => {
  ...
  hub.on('shutdown', () => { clearTimeout(handle) })
})
